def inventory():
    ...
    mycon=x.connect(host='localhost',user="root",passwd="cinnabar",database="proj")
    ...
    while True:
        ...
        if ch==1:#view all items
            ...
            if a=='asc':
                ...
                print("Itemid, Name, Itemcateg, Stock, Lastupd, Expdate, CP, SP")
                cursor.execute("Select * from inventory order by stock")
                data=cursor.fetchall()
                for row in data:
                    print(row)
                ans=input("Do you want to continue?(yes/no):")
            ...
            if y==1:
                ...
                while True:
                    ...
                    cursor.execute("insert into inventory values('{}','{}','{}',{},'{}','{}',{},{})".format(itid,itname,itcateg,itstock,itupd,itexp,itcp,itsp))
                    mycon.commit()
            ...
            while True:
                ...
                if y==1:
                    ...
                    cursor.execute("Update inventory set name='{}' where itemid='{}'".format(new,itid))
                    mycon.commit()
                    ...
                    cursor.execute("Update inventory set itemcateg='{}' where itemid='{}'".format(new,itid))
                    mycon.commit()
                    print("updated successfully")
                    ans=input("Do you want to modify another item?(yes/no):")
                    if ans=="yes":
                    ...
                    cursor.execute("Update inventory set stock={} where itemid='{}'".format(new,itid))
                    mycon.commit()
                    print("updated successfully")
                    ans=input("Do you want to modify another item?(yes/no):")
                    if ans=="yes":
                    ...
                    cursor.execute("Update inventory set lastupd='{}' where itemid='{}'".format(new,itid))
                    mycon.commit()
                    print("updated successfully")
                    ans=input("Do you want to modify another item?(yes/no):")
                    if ans=="yes":
                    ...
                    cursor.execute("Update inventory set expdate='{}' where itemid='{}'".format(new,itid))
                    mycon.commit()
                    print("updated successfully")
                    ans=input("Do you want to modify another item?(yes/no):")
                    if ans=="yes":
                    ...
                    cursor.execute("Update inventory set cp={} where itemid='{}'".format(new,itid))
                    mycon.commit()
                    print("updated successfully")
                    ans=input("Do you want to modify another item?(yes/no):")
                    if ans=="yes":
                    ...
                    cursor.execute("Update inventory set sp={} where itemid='{}'".format(new,itid))
                    mycon.commit()
                    print("updated successfully")
                    ans=input("Do you want to modify another item?(yes/no):")
                    if ans=="yes":
...
def supplier():
    ...
    while True:
        ...
        #view suppliers item wise
            ...
            cursor.execute("select inventory.itemid,inventory.name as 'itemname',ID,supplier.name as 'supplier name' from inventory,supplier,itemsup where inventory.itemid=itemsup.itemid and ID=supid;")
            data=cursor.fetchall()
            for row in data:
                print(row)
            ans=input("Do you want to continue?(yes/no):")
...
def orders():
    ...
    while True:
        ...
        if ch==1:
            ...
            cursor.execute("select orders.itemid,name,ordcnt,orddate from inventory,orders where inventory.itemid=orders.itemid order by orddate")
            data=cursor.fetchall()
            for row in data:
                print(row)
            ans=input("Do you want to continue?(yes/no):")
        ...
        #add or delete order
            ...
            if y==1:
                while True:
                    ...
                    cursor.execute("Insert into orders values('{}',{},'{}')".format(itid,curord,orddate))
                    mycon.commit()
                    print("successfully added")
                    ans=input("Do you want to add more?(yes/no):")
                    if ans=="yes":
